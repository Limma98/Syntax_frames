...
-- Specify, if the menu should be accessible by using the 'camera' item (ox_inventory).
-- The item is defined in ox_inventory/data/items.lua and calls this resource's
-- 'useCamera' export (client.export = 'cinematiccam.useCamera').
-- Set Config.useButton and Config.useCommand to false to make the item the ONLY way in.
Config.useItem = true
...
-- Screenshot button (uses the 'screencapture' resource + a Discord webhook).
-- The webhook URL itself is SERVER-SIDE only — set it in server.cfg with:
--     set syntax_frames:webhook "https://discord.com/api/webhooks/XXXX/YYYY"
-- Here you only toggle whether the button is usable at all.
Config.screenshot = {
    enable = true,
}
